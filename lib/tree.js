...
const buildTree = (root_id, tweets) => {
  ...
  // Determine the max ID for current batch of tweets. Since the tweets
  // are deleted while being processed, this should be done early on.
  const meta = {
    start_from_id: BigNumber.max(tweets.map(t => t.id_str)).toString()
  };
